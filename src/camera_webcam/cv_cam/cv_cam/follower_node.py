...
def timer_callback():
    ...
    # Apply a threshold to detect dark regions
    _, threshold = cv2.threshold(gray, 50, 255, cv2.THRESH_BINARY_INV)

    # Get the contours from the thresholded image
    contours, _ = cv2.findContours(threshold, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

    # Process the contours and determine the line centroid
    line, mark_side = get_contour_data(threshold, image[crop_h_start:crop_h_stop, crop_w_start:crop_w_stop])
    ...
    if line:
        ...
        cv2.circle(image, (line['x'], crop_h_start + line['y']), 5, (0, 255, 0), 7)
    ...
    if mark_side is not None:
        ...
        if (
            mark_side == "right"
            and finalization_countdown is None
            and abs(error) <= MAX_ERROR
            and not just_seen_right_mark
        ):
            right_mark_count += 1
    ...
    cv2.rectangle(image, (crop_w_start, crop_h_start), (crop_w_stop, crop_h_stop), (0, 0, 255), 2)

    # Show the output image to the user
    cv2.imshow("output", image)
    # Print the image for 5 milliseconds, then resume execution
    cv2.waitKey(5)
